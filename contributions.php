				...
				<table class="table table-bordered" id="contribution-pool">
					...
					<thead>
						<tr>
							...
							<th class="text-center">Contribution Details</th>
							<th class="text-center"> To Change </th>
					...
					<tbody>
						<?php
							...
							$i=1;
							$type = $conn->query("SELECT * FROM  projects where id in (SELECT project_id from contribution_list) ");
							...
							$qry = $conn->query("SELECT l.*,concat(b.lastname,', ',b.firstname,' ',b.middlename)as name, b.contact_no, b.address from contribution_list l inner join members b on b.id = l.contributor_id  order by id asc");
						 ...
						 <tr>
						 	...
						 	<td class="text-center">
						 			<button class="btn btn-outline-primary btn-sm edit_contribution" type="button" data-id="<?php echo $row['id'] ?>"><i class="fa fa-edit"></i></button>
...
<script>
	...
	$('.edit_contribution').click(function(){
		uni_modal("Edit Contribution","manage_contribution.php?id="+$(this).attr('data-id'),'mid-large')
	})
